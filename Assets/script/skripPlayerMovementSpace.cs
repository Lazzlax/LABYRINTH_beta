...
public class skripPlayerMovementSpace : MonoBehaviour {
	/*
	...
	//SoftLeaf
	private Rigidbody fisika;

	void Start()
	{
		anim = GetComponent<Animator> ();
		cameraTransform = Camera.main.transform;
		fisika = GetComponent<Rigidbody>();
	...
	void FlyManagement(float horizontal, float vertical)
	{
		...
		fisika.AddForce(direction * flySpeed * 100 * (sprint?sprintFactor:1));
	}

	void JumpManagement()
	{
		if (fisika.velocity.y < 10) // already jumped
		{
		...
		if (Input.GetButtonDown ("Jump"))
		{
			...
			{
				fisika.velocity = new Vector3(0, jumpHeight, 0);
	...
	void MovementManagement(float horizontal, float vertical, bool running, bool sprinting)
	{
		...
		fisika.AddForce(Vector3.forward*speed);
	}
...
*/
	...
	public float EvaSpeed;
	public float targetEvaSpeed;

	void Start(){
		targetEvaSpeed = EvaSpeed;
	}
	...
	public void AnimaTIOn(float x, float y){
		//get hashes
		//use locomotion x dan y
	}

	public void EVAcontrol(float x,float y){
		isMoving = Mathf.Abs(y)>0.1f;
	
		if(isMoving){
			EvaSpeed = targetEvaSpeed;
		}
		if(!isMoving){
			fisika.velocity = Vector3.zero;
			EvaSpeed = 0f;
		}
		Vector3 direction = Rotating(x, y);
		fisika.AddForce(direction * EvaSpeed);
	}
